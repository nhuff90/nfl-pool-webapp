...
def printGames(week, gameResults):
    for resultgame in gameResults:
        awayScore = ''
        homeScore = ''
        if isinstance(resultgame, dict):
            gameWithScore = nflgame.one(year=2019, week=week, home=str(resultgame['home']),
                                        away=str(resultgame['away']))
            if gameWithScore is not None:
                awayScore = str(gameWithScore.score_away)
                homeScore = str(gameWithScore.score_home)
            print('{\"eid\": \"' + str(resultgame['eid']) + '\", \"week\": \"' + str(
                resultgame['week']) + '\", \"homeTeam\": \"' + str(
                resultgame['home']) + '\", \"homeScore\": \"' + homeScore + \
                  '\", \"awayTeam\": \"' + str(resultgame['away']) + '\", \"awayScore\": \"' + awayScore + '\"}')
        else:
            gameWithScore = nflgame.one(year=2019, week=week, home=str(resultgame.home), away=str(resultgame.away))
            if gameWithScore is not None:
                awayScore = str(gameWithScore.score_away)
                homeScore = str(gameWithScore.score_home)
            print('{\"eid\": \"' + str(resultgame.eid) + '\", \"week\": \"' + str(
                resultgame.schedule['week']) + '\", \"homeTeam\": \"' + str(
                resultgame.home) + '\", \"homeScore\": \"' + homeScore + \
                  '\", \"awayTeam\": \"' + str(resultgame.away) + '\", \"awayScore\": \"' + awayScore + '\"}')
...
results = []
games = live._games_in_week(2019, week=num)
...
for g in games:
    ...
    if active is False and completed is False:
        ...
        if g is not currentGames and game.Game(g['eid']) == None or game.Game(g['eid']).time.qtr == "Pregame":
            results.append(g)
